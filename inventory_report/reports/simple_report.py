...
class SimpleReport:
    @staticmethod
    def __findOldDate(element):
        return datetime.strptime(
            element["data_de_fabricacao"], "%Y-%m-%d").date()

    @staticmethod
    def __findRecentDate(element):
        return datetime.strptime(
            element["data_de_validade"], "%Y-%m-%d").date()

    @staticmethod
    def __findName(element):
        return element["nome_da_empresa"]

    @classmethod
    def generate(cls, dictionary):
        fabricacaoMaisVelha = list(map(cls.__findOldDate, dictionary))
        ...
        empresaMaisEstoque = list(map(cls.__findName, dictionary))
