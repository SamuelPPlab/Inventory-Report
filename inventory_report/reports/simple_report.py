...
class SimpleReport:
    ...
    def generate(cls, data):
        ...
        list_manufacturing_date = [item["data_de_fabricacao"] for item in data]
        list_expiration_date = [
            item["data_de_validade"]
            for item in data
            if item["data_de_validade"] > str(date_actual)
        ]
        list_companies = [item["nome_da_empresa"] for item in data]

        report = ("Data de fabricação mais antiga: "
                  + min(list_manufacturing_date) + "\n"
                  + "Data de validade mais próxima: "
                  + min(list_expiration_date) + "\n"
                  + "Empresa com maior quantidade de produtos estocados: "
                  + statistics.mode(list_companies)) + "\n"
...
#             {
#                 "id": "5",
#                 "nome_do_produto": "ACETAMINOPHEN, PHENYLEPHRINE HYDROCHLOR",
