    ...
    def generate(dataReport: list) -> str:
        new_data = list(
            OrderedDict.fromkeys(
                [report["nome_da_empresa"] for report in dataReport]
            )
        )
        prods_stocked = {}
        for company in new_data:
            prods_stocked[company] = 0

        for report in dataReport:
            prods_stocked[report["nome_da_empresa"]] += 1

        products = "Produtos estocados por empresa: \n"

        for company in new_data:
            products += f"- {company}: {prods_stocked[company]}\n"
        return f"{SimpleReport.generate(dataReport)}\n" f"{products}"
